...
if ($psd1Content -match "RequiredVersion\s+=\s+'(.*?)'\s+# Generated by Build.ps1") {
    "Found `Microsoft.PowerShell.ConsoleGuiTools` RequiredVersion in the old .psd1 file: " + $Matches[1]
    ...
    if ($localRepository) {
        ...
        if ($ocgvVersion -match "$ocgvModule.(.*?).nupkg") {
            ...
            throw "RequiredVersion not found in $PsdPath."
        }
        "Latest `Microsoft.PowerShell.ConsoleGuiTools` in 'local' repository: " + $ocgvVersion
